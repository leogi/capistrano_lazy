...
app_directory = "<%= @deploy_directory %>/current"
working_directory app_directory # available in 0.94.0+
listen_port = <%= @unicorn_port %>

# listen on both a Unix domain socket and a TCP port,
# we use a shorter backlog for quicker failover when busy

listen "#{app_directory}/tmp/sockets/<%= @sock_file %>", :backlog => 128
...
pid "#{app_directory}/tmp/pids/<%= @pid_file %>"
